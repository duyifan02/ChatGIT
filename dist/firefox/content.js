  ...
  class Controller {
    ...
    renderTree() {
      ...
      for (const { node, guides } of order) {
        ...
        textBtn.textContent = text.length > 60 ? text.slice(0, 57) + "…" : text;
    ...
    scrollToMsg(index, smooth) {
      ...
      const targetTop = clamp(msg.top - Math.floor(vh * SCROLL_ANCHOR), 0, maxTop);

      msg.element.scrollIntoView({ block: "start", behavior: smooth ? "smooth" : "auto" });
      ...
      this.jumpTimer = window.setTimeout(() => {
        if (isDoc) {
          ...
          sc.scrollTo({ top: targetTop, behavior: smooth ? "smooth" : "auto" });
        }
        this.updateActive();
      }, smooth ? 140 : 0);
